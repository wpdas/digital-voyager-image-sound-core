/*!
 * bitmap (encode and decode)
 * Copyright(c) 2020 wenderson <wendersonpdas@gmail.com>
 * MIT Licensed
 *
 * Copied from: https://github.com/ericandrewlewis/bitmap-js/blob/master/index.js
 * and adapted to this project
 *
 * Original git repo: https://github.com/ericandrewlewis/bitmap-js
 *
 * Open a PR sending this TS version
 */

import { readFile, writeFile } from 'fs';

interface IPadImageData {
  unpaddedImageData: Buffer;
  width: number;
  height: number;
}

interface IDibHeader {
  width: number;
  height: number;
  bitsPerPixel: number;
...
function readDibHeader(filedata: Buffer) {
  ...
  header.headerType = dibHeaderLengthToVersionMap.get(dibHeaderLength);
